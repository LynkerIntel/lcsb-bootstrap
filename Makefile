...
upload:
	...
	$(eval BUCKET := $(shell tofu output -raw bucket_name))
	$(eval BPATH := tfstate/csb-bootstrap/terraform.tfstate)
	@echo "Using bucket: $(BUCKET)"
	@echo "Uploading state file to target location... $(BUCKET)/$(BPATH)"
	aws s3 cp terraform.tfstate s3://$(BUCKET)/$(BPATH)
